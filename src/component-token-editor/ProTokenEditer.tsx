...
import ComponentDemos from '../previews/procomponents';
...
const ProComponentTokenEditor: FC<ComponentTokenEditorProps> = ({ theme }) => {
  const locale = useLocale();
  const prefixCls = `antd-component-token-editor`;
  const [activeComponent, setActiveComponent] = useState<string>('ProTable');
  ...
  const menuItems: MenuProps['items'] = useMemo(
    () =>
      Object.entries(ProComponentsList).map(([key, value]) => ({
        key,
        label: key,
        type: 'group',
...
export default ProComponentTokenEditor;
